...
{
    if (n < 3) {
        return -1; // Массив слишком короткий для поиска локального максимума
    }

    long long min_local_max = LLONG_MAX; // Начальное значение для минимального локального максимума

    for (size_t i = 1; i < n - 1; ++i) {
        if ((A[i] > A[i - 1]) && (A[i] > A[i + 1])) {
            // Элемент является локальным максимумом
    ...
    if (min_local_max == LLONG_MAX) {
        return -1; // Локальные максимумы отсутствуют
    }
